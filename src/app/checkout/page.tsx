...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Step 2: Send WhatsApp message with order details
      ...
      // Step 3: Update order with WhatsApp message details
      if (whatsappResult.success && whatsappResult.messageId) {
        console.log("📲 Updating order with WhatsApp message ID...");

        try {
          await fetch(`/api/orders/${savedOrder.id}/status`, {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
            },
            body: JSON.stringify({
              ...
              whatsappStatus: "sent",
              notes: "WhatsApp order confirmation sent to customer",
            }),
          });
          console.log("✅ Order updated with WhatsApp details");
        } catch (updateError) {
          console.warn(
            "⚠️ Failed to update order with WhatsApp details:",
            updateError
          );
        }
      }
...
Please check your WhatsApp for complete order details and next steps.`;
        ...
        throw new Error(
          whatsappResult.message || "Failed to send WhatsApp confirmation"
        );
      }
    } catch (error: any) {
